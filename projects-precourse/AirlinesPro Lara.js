...
function deleteIdFlight() {
  ...
  let flightToDelete = parseInt(
    prompt(`Indique el id del vuelo que desea eliminar:`)
  );
  if (
    flightToDelete == "" ||
    isNaN(flightToDelete) ||
    flightToDelete === null
  ) {
...
function seeActualFlights() {
  ...
  for (let i = 0; i < flights.length; i++) {
    ...
    if (ids.scale === false) {
      console.log(
        `${[i + 1]}) Vuelo con Id:${ids.id}, con origen ${ids.to} y destino ${
          ids.from
        }. \nTiene un coste de: ${ids.cost}€ / No realiza ninguna escala.`
      );
    } else {
...
function searchByPrice() {
  let flightsPrice = parseInt(
    prompt(
      `Indique cuál es su presupuesto para buscarle el vuelo más adecuado:`
    )
  );
  if (flightsPrice == "" || isNaN(flightsPrice) || flightsPrice === null) {
    ...
    for (let i = 0; i < flights.length; i++) {
      if (flightsPrice >= flights[i].cost) {
        console.log(
          `- Vuelo con origen ${flights[i].from} y destino ${flights[i].to} \nPrecio: ${flights[i].cost}€ `
        );
      }
    }
  }
